...
|shift | shift ctrl alt altgr meta super none, or a key name such as hangul | Map the server's shift modifier to different key on a client computer|
|ctrl  | shift ctrl alt altgr meta super none, or a key name such as hangul | Map the server's ctrl modifier to different key on a client computer|
|alt | shift ctrl alt altgr meta super none, or a key name such as hangul | Map the server's alt modifier to different key on a client computer|
|meta|  shift ctrl alt altgr meta super none, or a key name such as hangul | Map both of the server's meta modifiers to a different key on a client computer|
|metaLeft|  shift ctrl alt altgr meta super none, or a key name such as hangul | Map the server's left meta modifier to a different key on a client computer|
|metaRight|  shift ctrl alt altgr meta super none, or a key name such as hangul | Map the server's right meta modifier to a different key on a client computer|
|super|  shift ctrl alt altgr meta super none, or a key name such as hangul | Map both of the server's super modifiers to a different key on a client computer|
|superLeft|  shift ctrl alt altgr meta super none, or a key name such as hangul | Map the server's left super modifier to a different key on a client computer|
|superRight|  shift ctrl alt altgr meta super none, or a key name such as hangul | Map the server's right super modifier to a different key on a client computer|
